        ...
        currentImageUri?.let { uri ->
            ...
            lifecycleScope.launch {
                try {
                    ...
                    successResponse.errorCode?.let { code ->
                        if (code != 0) {
                            showToast("Error: $code - ${successResponse.message}")
                            showLoading(false)
                            return@launch
                        }
                    }
                    ...
                    successResponse.data?.let { responseData ->
                        binding.resultTextView.text = if (responseData.isAboveThreshold == true) {
                            showToast(successResponse.message ?: "Upload successful")
                            String.format("%s with %.2f%%", responseData.result, responseData.confidenceScore)
                        } else {
